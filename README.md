...
服务启动后同时保存 UTF-8 `.log` 文本和 JSONL 日志，各按 10 MB 分卷；读取、历史转换与模型失败诊断见[开发计划第 10.5 节](docs/Credra%20Agent%20自主调查架构开发计划%20v2.0.md#105-可读文本日志诊断与实施验证)。更新后需重启根服务加载新日志实现。

V2-2/P25 新增自然语言 UI 执行开关和可信运行策略：配置后自动解析/澄清并启动 Agent，解析与调查共享预算，支持状态/恢复；开启方法见 [开发计划第 6.1.1 节](docs/Credra%20Agent%20自主调查架构开发计划%20v2.0.md#611-ui-执行配置与使用)，进度与验证边界见 [第 6.1.2 节实施与验收记录](docs/Credra%20Agent%20自主调查架构开发计划%20v2.0.md#612-p25-实施与验收记录)。真实试跑预算、P24/P23 财务与复杂首例联合验收仍待完成。

全服务日志、启动归档、10 MB 轮转和日志故障后的安全恢复，见[日志使用说明](docs/全服务日志使用说明.md)。当前开发进度与验证记录见 [V2-1 实施记录](docs/V2-1%20实施与验证记录.md)。

V2-2 的 P22 技术验证已完成并提交，见 [P22 实施记录](docs/V2-2%20P22%20证据驱动重规划实施记录.md)。agentic 调查根据原文、主张及冲突选择读取、抓取、核验或独立检索，核验与 Coordinator 共享运行预算；完成问题需通过当前证据引用门禁。已确认保留 thinking 摘要和独立 JSON 请求，预算包含两段及重试。P24 财务计算与 P23 复杂案例联合验收尚未完成。[P21 实施记录](docs/V2-2%20P21%20证据与实体处理实施记录.md)保留证据基础设计和验证记录。
