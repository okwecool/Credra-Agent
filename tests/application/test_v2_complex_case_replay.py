...
from credra_agent.evidence.investigation import PROMPT_VERSION, VERIFIER_PROMPT
...
from tests.financial.test_v2_financial import (
    FinancialCitation,
    dataset,
    financial_task,
    report_fixture,
...
def test_coordinator_prompt_bounds_finish_projection():
    ...
    assert "不得用背景法规、行业资料或同一事项的改写凑数" in prompt
    assert "claim_proposals.finding_aspect 必须原样填写" in prompt


def test_verifier_prompt_keeps_excerpt_and_location_in_one_fragment():
    assert PROMPT_VERSION == "evidence-verifier-v3-single-fragment-grounding"
    assert "excerpt 和 location 必须精确对应同一个给定正文片段" in VERIFIER_PROMPT
    assert (
        "entity_excerpt 和 entity_location 可以独立对应另一个单一片段"
        in VERIFIER_PROMPT
    )
...
def test_document_actions_ignore_only_rotating_bundle_reference():
    ...
    read_first = canonical_action_signature(
        "read_document",
        {
            "reference_id": "artifacts/agent_evidence_v1.json",
            "document_id": "source-a",
        },
    )
    read_rotated = canonical_action_signature(
        "read_document",
        {
    ...
    read_other = canonical_action_signature(
        "read_document",
        {
            "reference_id": "artifacts/agent_evidence_v2.json",
    ...
    assert read_first == read_rotated and read_first != read_other
...
def test_agentic_quality_check_requires_verified_settlement_finding(tmp_path):
    ...
    report["findings"][0]["question_id"] = "q-settlement"

    checks = check_report(
        report,
        package,
        required_finding_counts={"q-settlement": 3},
    )
    ...
    seed = report["findings"][0]
    report["findings"] = [
        {**seed, "claim_id": f"settlement-claim-{index}"} for index in range(3)
    ]
    checks = check_report(
        report,
        package,
    ...
    assert not any(
        item["check_id"] == "evidence.required_question:q-settlement" for item in checks
    )
    assert {
        "check_id": "evidence.required_receipt:settlement-anonymous-shift",
        "status": "FAIL",
        "reason": "MISSING_REQUIRED_RECEIPT",
    } in checks
    assert {
        "check_id": "evidence.required_receipt:settlement-company-response",
        "status": "FAIL",
        "reason": "MISSING_REQUIRED_RECEIPT",
    } in checks
...
def test_frozen_human_pack_does_not_auto_score_or_invent_real_counterexamples():
    ...
    settlement = next(
        item for item in package["questions"] if item["question_id"] == "q-settlement"
    )
    explanation = next(
        item for item in package["questions"] if item["question_id"] == "q-explanation"
    )
    assert explanation["required_finding_aspects"] == ["年报现金流变化解释"]
    assert settlement["minimum_verified_findings"] == 3
    assert settlement["required_finding_aspects"] == [
        "付款承诺",
        "匿名来源结算调整报道",
        "公司回应",
    ]
...
def test_real_trial_pool_preflights_full_request_token_bound(tmp_path):
    from app.llm.gateway import StructuredModelError
    ...
    model.request_reservation_multiplier = model.token_reservation_multiplier = 1
    authorization = offline_authorization().model_copy(update={"token_limit": 1_000})
    pool = PooledTrialModel(model, tmp_path, authorization)

    with pytest.raises(StructuredModelError, match="shared budget exhausted"):
        pool.generate(
            output_schema=DecisionDraft,
            purpose="offline-preflight-check",
            prompt_version="test-v1",
            system_prompt="必须完整覆盖结构化输入。" * 100,
            payload={"evidence_context": {"document_reads": []}},
...
def test_actual_ui_message_reaches_same_task_evidence_calculations_and_report(
    ...
    from tests.entry.test_v2_entry_delegation import decision, draft
    from tests.entry.test_v2_entry_delegation import settings as old_settings
