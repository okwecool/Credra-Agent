...
from credra_agent.planning.coordinator import Coordinator, _proposes_missing_finding
...
def test_new_proposal_for_missing_finding_can_escape_no_progress_lock():
    draft = DecisionDraft(
        decision="ACTION",
        tool="verify_claim",
        arguments={
            "claim_id": "proposal:company-response",
            "source_refs": ["artifacts/agent_evidence_v1.json"],
            "document_ids": ["response-document"],
        },
        expected_observation="核验缺失的公司回应。",
        reason_summary="补齐公司回应。",
        claim_proposals=[
            {
                "proposal_id": "proposal:company-response",
                "question_id": "q-regulatory",
                "statement": "公司回应其结算安排符合现行规定。",
                "kind": "PARTY_STATEMENT",
                "attributed_to": "比亚迪",
                "source_document_ids": ["response-document"],
                "finding_aspect": "公司回应",
            }
        ],
    )
    context = {
        ...
        "completion_progress": [
            {
                "question_id": "q-regulatory",
                "required_finding_aspects": ["付款承诺", "公司回应"],
                "missing_finding_aspects": ["公司回应"],
                "minimum_verified_findings": 2,
                "verified_finding_claim_ids": ["proposal:pledge"],
            }
        ],
    }

    assert _proposes_missing_finding(draft, context)
    context["claim_proposals"] = [{"proposal_id": "proposal:company-response"}]
    assert not _proposes_missing_finding(draft, context)
...
def test_verify_claim_policy_requires_evidence_bundle_reference():
    with pytest.raises(PolicyViolation) as error:
        ActionPolicy(default_registry()).authorize(
            tool="verify_claim",
            arguments={
                "claim_id": "proposal:company-response",
                "source_refs": ["artifacts/agent_tool_result_v3.json"],
                "document_ids": ["response-document"],
            },
            task_spec=task_spec(allowed=["exchange", "regulator"]),
            executable_tools={"verify_claim"},
            available_refs={"artifacts/agent_tool_result_v3.json"},
            prior_signatures=set(),
        )
    assert error.value.code == "INVALID_REFERENCE_TYPE"


def test_duplicate_document_read_returns_actionable_policy_code():
    policy = ActionPolicy(default_registry())
    arguments = {
        "reference_id": "artifacts/agent_evidence_v2.json",
        "document_id": "annual-report",
    }
    signature = policy.authorize(
        tool="read_document",
        arguments=arguments,
        task_spec=task_spec(allowed=["exchange", "regulator"]),
        executable_tools={"read_document"},
        available_refs={"artifacts/agent_evidence_v2.json"},
        prior_signatures=set(),
    ).signature
    with pytest.raises(PolicyViolation) as error:
        policy.authorize(
    ...
    assert error.value.code == "DOCUMENT_ALREADY_READ"
...
@pytest.mark.parametrize(
    ("second_finish", "expected_reason"),
    [
        (finish_decision(), "INCOMPLETE_COMPLETION_REVIEW_REQUIRED"),
        (
...
def test_incomplete_second_finish_requires_explicit_review(
    tmp_path, second_finish, expected_reason
):
    incomplete = task_spec(allowed=["exchange", "regulator"])
    incomplete = incomplete.model_copy(
        update={
            "questions": [
                incomplete.questions[0].model_copy(
                    update={"minimum_verified_findings": 1}
                )
            ]
        }
    )
    model = FixedModel([finish_decision(), second_finish])

    result = coordinator(tmp_path, model, lambda _: None).run(incomplete)

    assert result.status == "LIMITED" and result.stop_reason == expected_reason
    assert len(model.payloads) == 2
    assert model.payloads[1]["policy_rejections"][0]["code"] == (
        "INCOMPLETE_COMPLETION_REQUIREMENTS"
    )
    if expected_reason == "NEEDS_REVIEW":
        assert result.coverage.review_required
