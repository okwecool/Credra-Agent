    ...
    [
        ...
        (
            DecisionDraft(
                ...
                finish_reason="NEEDS_REVIEW",
                review_required=True,
                reason_summary="结构化完成条件仍有缺口，转人工复核。",
            ),
            "NEEDS_REVIEW",
        ),
    ],
)
def test_durable_graph_requires_review_for_second_incomplete_finish(
    tmp_path, second_finish, expected_reason
):
    incomplete = task_spec()
    ...
    result = start_agentic_task(
        thread_id="incomplete-finish-review-gate-" + expected_reason.lower(),
    ...
    state = result["state"]
    assert state["status"] == "LIMITED" and state["stop_reason"] == expected_reason
    assert model.calls == 2
    if expected_reason == "INCOMPLETE_COMPLETION_REVIEW_REQUIRED":
        assert state["policy_rejections"][-1]["code"] == expected_reason
...
def run_recovery_cli(root: Path, command: str, thread_id: str, crash_at=None):
    arguments = [
        sys.executable,
        "-m",
        "tests.support.agentic_recovery_cli",
        command,
        "--root",
        str(root),
        "--thread-id",
