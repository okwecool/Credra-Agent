...
from tests.agentic.test_v2_evidence import (
    AS_OF,
    CHILD,
    PARENT,
    bundle,
...
def test_model_cannot_forge_receipt_or_rebind_proposal(tmp_path):
    ...
    aspect_task = ctx.task_spec.model_copy(
        update={
            "questions": [
                ctx.task_spec.questions[0].model_copy(
                    update={
                        "minimum_verified_findings": 1,
                        "required_finding_aspects": ["公司回应"],
                    }
                )
            ]
        }
    )
    with pytest.raises(ValueError, match="required finding aspect"):
        store_proposals(
            ctx.artifacts,
            [
                proposal.model_copy(
                    update={
                        "proposal_id": "proposal:missing-aspect",
                        "finding_aspect": None,
                    }
                )
            ],
...
def test_source_policy_blocks_fetch_before_callback_and_read_truncates(tmp_path):
    ...
    assert len(visible["document_reads"]) == 1
    assert visible["read_document_ids"] == ["original"]
    assert "verified_claim_index" in visible
    assert all(
