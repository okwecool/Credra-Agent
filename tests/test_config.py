...
def test_settings_have_safe_local_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert settings.fact_verifier_enable_thinking is None
...
def test_fact_verifier_thinking_override_loads_from_environment(
    ...
    monkeypatch.setenv("FACT_VERIFIER_ENABLE_THINKING", "false")

    settings = Settings(_env_file=None)

    assert settings.fact_verifier_enable_thinking is False
...
def test_settings_reject_invalid_thresholds_and_retry_count() -> None:
    with pytest.raises(ValidationError):
        Settings(
            ...
            analysis_llm_research_max_output_tokens=10,
        )
