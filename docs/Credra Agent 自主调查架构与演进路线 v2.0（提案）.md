...
日期：2026-09-07。状态：**架构方向已于 2026-09-07 获用户认可，V2 功能尚未实现。实施工作包与阶段门禁见 [自主调查架构开发计划 v2.0](Credra%20Agent%20自主调查架构开发计划%20v2.0.md)。**

本提案依据编制时的工作区代码、MVP 需求与开发计划、场景化能力优化计划及开发日志，响应“增加 LLM 决策参与、支持自然语言指导、保留默认/降级链路、提高案例复杂度”的要求。路线编制阶段已完成合成技术样例退出默认入口，比亚迪和上汽保留。用户现已认可架构并要求先形成开发计划；代码实施尚未启动。

**后续确认（2026-09-07）：** 首个复杂真实案例先深化比亚迪或上汽，再新增企业；首例所需的最小应收/现金质量字段与计算前置到 V2-2，其余财务能力仍在 V2-3；不要求准确工时估算，按验证节点推进并允许用户随时调整。以下原始路线的工时表及阶段分配保留为设计历史，实际执行以新开发计划为准。

**追加优化（2026-09-07）：** 纳入全服务日志；用户确认整套服务（含 MCP 子进程）共用一个启动批次，单文件按 10 MB 轮转。该能力安排在 V2-0 设计、V2-1 优先实现，详见第 7.4 节及开发计划第 10.4 节。
...
credra_agent/
  ...
  observability/  # 全服务日志、启动批次、进程汇聚、轮转与脱敏
...
新增覆盖入口、服务生命周期、Runtime、Graph/Agent、LLM、MCP、搜索/抓取/核验、Checkpoint、Artifact/报告生成及异常的统一日志。当前 `app/runtime/tracing.py` 仅按任务写入 JSONL，不具备整套服务启动归档或容量轮转，不能替代这项建设。

- 每次根服务启动建立唯一 startup_id 和 `logs/<startup_id>/`，生成首个 `service.0001.jsonl` 及启动清单；同批任务和 MCP 子进程共用该批次。
- 文件按 UTF-8 实际字节数计量，10 MB 明确定义为 10,000,000 字节；追加完整记录将超过上限时，先另起连续编号的新分卷，旧启动与旧分卷不覆盖。
- 主进程单一写入者汇聚子进程事件并负责轮转；MCP stdout 保持协议专用，CLI 的结构化结果也不能被日志污染。Windows 进程通信在 P04 做 Spike 验证。
- 必记 Agent 节点进入/结束/失败/跳过及路由变化、任务状态迁移前后值与原因、LLM 每次 attempt 的请求结果、HTTP/流状态、解析/Schema/引用校验、重试/降级及最终状态。
- 通过 startup_id、进程、任务/Run、节点、Action、call_id 和 attempt 串联事件；多个任务不会因共享服务而混淆。同一任务跨启动恢复可串联新旧批次。
- 日志记录状态和受限诊断元数据，禁止真实密钥、认证头、完整 Prompt、原始响应/思维链或抓取全文进入日志；超长消息有明确截断/分片标记。
- 正常退出刷新日志并记录关闭；强制终止和写入失败明确完整性边界。磁盘/汇聚异常不能静默吞掉；保留策略如涉及删除旧文件需另行确认。
- 现有任务 Trace、Checkpoint 与 Artifact 语义保留，通过统一事件关联到服务日志；日志不成为新的业务状态存储。

实施与验收以[开发计划](Credra%20Agent%20自主调查架构开发计划%20v2.0.md)的 P04/P15、A17 和 L01–L08 为准，先覆盖现有 baseline 服务，再接入自主循环。该项目前仅写入设计与计划，未实施日志代码。
...
7. 仅按任务记录业务 Trace → 增加全服务启动归档、10 MB 日志轮转及节点/状态/模型结果等事件记录；保留既有 Trace 用途。
...
本节保留当时的文件范围和审核边界；本次编制新开发计划的改动清单见新计划第 12 节，不以本节的暂存状态描述当前工作区。
