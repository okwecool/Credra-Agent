...
### M5-A2：上汽集团第二套公开业务 Case

第二套 Case 用于验证 Eval 方法和数据血缘并非只适配比亚迪，按以下边界实施：

- 主体固定为上海汽车集团股份有限公司，上交所 A 股代码 `600104`，Case ID 为 `case_saic_600104`；不得把控股股东上海汽车工业（集团）有限公司、单一合资企业或品牌口径混作上市公司合并口径；
- 观察期固定为 2023—2025 连续三个完整会计年度，优先使用上交所披露的年度报告，必要时以公司投资者关系页面交叉核验；仓库只保存结构化字段、来源 URL、页码、表名、口径和必要摘要，不保存完整 PDF；
- 财务字段继续使用现有统一 Schema：营业收入、归母净利润、流动资产、流动负债、资产总额、负债总额和经营活动现金流量净额，统一归一到 `CNY_1000`；跨年比较优先采用最新年度报告中的经重述比较数，并在 Source Manifest 中逐字段记录来源版本和会计口径；
- 先完成人工来源清单、Company/Financial 输入、字段血缘和 Case Validation，再运行确定性主链路形成预期 Financial/Anomaly/Risk 基线；基线必须由人工审核后才能固化进第二个 Eval Suite；
- 第二套 Eval 仍使用正式 Durable Runtime 和固定离线 Provider，不调用搜索或模型；除财务、异常、风险、HITL 和报告边界外，增加“上交所来源与深交所来源均能进入同一通用 Case Schema”的兼容性检查；
- 吉利汽车控股有限公司作为第三套港交所对照 Case，代码固定为 `00175`，观察期同为 2023—2025；其 HKFRS/IFRS 披露格式、港交所来源和上市主体边界在上汽 Case 通过后另行建立适配门禁，不在 A2 中静默套用 A 股来源假设。

M5-A2 阶段门禁：所有输入字段均可追溯到公开报告位置；单位换算和比较口径通过独立计算复核；Case Validation 无 ERROR；正式主链路可在零外部调用下完成；上汽人工预期经用户审核前不得作为稳定业务真值提交；比亚迪 Suite 与全量回归继续通过。

实施状态（2026-09-06）：

- 已引入 case_saic_600104 的 Company、Financial、Business Info 和 Source Manifest 候选材料；2025 年数据及 2024 年比较数以交易所披露的 2025 年年度报告为主，2023 年资产负债表字段使用公司官网 2024 年年度报告的比较数，2023 年报告作为交叉核验来源；
- 已对 2025 年报告第 5、65、66 页和 2024 年报告第 68、69 页进行 PDF 图像复核；源表人民币元除以 1,000 后保存为 CNY_1000，并保留源表分币精度；
- Case Validation 已达到 VALID，确定性基线表现为无异常路由标记、风险等级 MEDIUM、现金流趋势一项 MEDIUM 风险；`expected_financial`、`expected_anomalies`、`expected_risk` 和人工验收清单已经版本化固化；
- 已新增默认双 Case Suite `auto_manufacturers_v1`，以同一正式 Durable Runtime 离线执行比亚迪与上汽，并增加可选的市场和主来源 Host 契约，验证深交所/上交所来源可进入同一通用 Case Schema；默认运行 32/32 检查通过且 `external_call_count=0`，原比亚迪单 Case Suite 保持 16 项检查可独立复跑；
- 上汽尚无经人工确认、可复现的无关证据负样本，当前不虚构该评分项；吉利汽车港股对照 Case 继续排在其后。
