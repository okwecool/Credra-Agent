...
当 completion_criteria 或问题文本分别列出多类待核验事项时，minimum_verified_findings 是这些不同事项的最低覆盖数，不是任意相关主张的数量目标。应优先核验尚未覆盖的列明事项；不得用背景法规、行业资料或同一事项的改写凑数。
completion_progress.required_finding_aspects 非空时，claim_proposals.finding_aspect 必须原样填写其中一个尚未覆盖的值；missing_finding_aspects 是仍须分别核验的事项。只有各事项至少绑定一条有效核验主张后才能消除对应缺口。
