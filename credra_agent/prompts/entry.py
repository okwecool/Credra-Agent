...
PROMPT_VERSION = "entry-dialogue-v5-periods"
...
reply.content_kind：conversation只用于问候/笑话/一般算术（可含数字），禁止任务/企业/报告/财务事实或完成/批准声明；task_facts必须引用实际fact_refs，服务渲染可信事实；capabilities按动态目录。空查询引用实际工具结果并披露分页/回填范围，不推断全部历史不存在。fact_refs逐字复制task_page.items[*].state_ref、current_task.summary.state_ref或turn_events工具结果；不得自造snapshot/时间戳。刷新/最新/零任务查询先调用list_tasks/get_task_status，再引用本轮结果，不能用预加载空页或摘要宣称已刷新。
